...
BEGIN
    ...
    (
      ...
      AUTO_INCREMENT = 1
      DEFAULT CHARSET = utf8 COMMENT ='version';
...
BEGIN
    ...
    THEN
        ALTER TABLE t_escheduler_task_instance
            ADD COLUMN `worker_group_id` int(11) NULL DEFAULT -1 COMMENT 'worker group id' AFTER `task_instance_priority`;
    END IF;
END;
...
BEGIN
    ...
    THEN
        ALTER TABLE t_escheduler_command
            ADD COLUMN `worker_group_id` int(11) NULL DEFAULT -1 COMMENT 'worker group id' AFTER `process_instance_priority`;
    END IF;
END;
...
BEGIN
    IF NOT EXISTS(SELECT 1
                  ...
                  WHERE TABLE_NAME = 't_escheduler_process_instance'
                    ...
                    AND COLUMN_NAME = 'worker_group_id')
    THEN
        ALTER TABLE t_escheduler_process_instance
            ADD COLUMN `worker_group_id` int(11) NULL DEFAULT -1 COMMENT 'worker group id' AFTER `process_instance_priority`;
    END IF;
END;
...
BEGIN
    ...
    THEN
        ALTER TABLE `t_escheduler_process_instance`
            ADD COLUMN `timeout` int(11) NULL DEFAULT 0 COMMENT 'time out' AFTER `worker_group_id`;
    END IF;
END;
...
BEGIN
    IF NOT EXISTS(SELECT 1
                  ...
                  WHERE TABLE_NAME = 't_escheduler_process_definition'
                    ...
                    AND COLUMN_NAME = 'timeout')
    THEN
        ALTER TABLE `t_escheduler_process_definition`
            ADD COLUMN `timeout` int(11) NULL DEFAULT 0 COMMENT 'time out' AFTER `create_time`;
    END IF;
END;
