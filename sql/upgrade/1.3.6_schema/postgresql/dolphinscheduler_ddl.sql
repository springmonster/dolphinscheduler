...
CREATE
OR
REPLACE FUNCTION uc_dolphin_T_t_ds_worker_group_A_ip_list() RETURNS void AS $$
...
ALTER TABLE t_ds_worker_group
    ALTER COLUMN addr_list type text;
ALTER TABLE t_ds_worker_group
    ALTER COLUMN name type varchar (255), ALTER COLUMN name SET NOT NULL;
ALTER TABLE t_ds_worker_group
...
CREATE
OR
REPLACE FUNCTION uc_dolphin_T_qrtz_fired_triggers_A_entry_id() RETURNS void AS $$
BEGIN IF EXISTS (SELECT 1 FROM information_schema.COLUMNS
        WHERE TABLE_NAME='qrtz_fired_triggers'
        AND COLUMN_NAME ='entry_id')
    THEN
ALTER TABLE qrtz_fired_triggers
    ALTER COLUMN entry_id type varchar (200);
END IF;
END;
$$ LANGUAGE plpgsql;
d//
...
ALTER TABLE t_ds_task_instance
    ADD CONSTRAINT foreign_key_instance_id FOREIGN KEY (process_instance_id) REFERENCES t_ds_process_instance (id) ON DELETE CASCADE;