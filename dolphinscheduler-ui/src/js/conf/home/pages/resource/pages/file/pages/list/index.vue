  ...
  <m-list-construction :title="$t('File Manage')">
      ...
      <m-conditions @on-conditions="_onConditions">
        <template slot="button-group">
          <x-button-group size="small">
            <x-button type="ghost" @click="() => $router.push({name: 'resource-file-createFolder'})">
              {{ $t('Create folder') }}
            </x-button>
            <x-button type="ghost" @click="() => $router.push({name: 'resource-file-create'})">{{ $t('Create File') }}
            </x-button>
    ...
    <template slot="content">
      <template v-if="fileResourcesList.length || total>0">
        <m-list @on-update="_onUpdate" :file-resources-list="fileResourcesList" :page-no="searchParams.pageNo"
