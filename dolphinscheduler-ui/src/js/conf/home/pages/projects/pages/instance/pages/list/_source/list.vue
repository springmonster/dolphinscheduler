    ...
    <div class="table-box">
      <table class="fixed">
        ...
        <tr v-for="(item, $index) in list" :key="item.id">
          <td width="50">
            <x-checkbox v-model="item.isCheck"
                        :disabled="item.state === 'RUNNING_EXECUTION' || item.state === 'READY_STOP' || item.state === 'READY_PAUSE'"
          ...
          <td style="min-width: 200px;max-width: 300px;padding-right: 10px;">
            <span class="ellipsis" style="padding-left: 4px;"><router-link
              :to="{ path: `/projects/${projectId}/instance/list/${item.id}` , query:{id: item.processDefinitionId}}"
              tag="a" class="links" :title="item.name">{{ item.name }}</router-link></span>
          </td>
          <td>
