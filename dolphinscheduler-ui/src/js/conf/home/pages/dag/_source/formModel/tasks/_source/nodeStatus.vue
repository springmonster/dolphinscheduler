    ...
    <div v-for="(el,$index) in dependItemList" :key='$index' class="list" @click="itemIndex = $index">
      ...
      <template v-if="isInstance">
        <span class="instance-state">
          ...
          <em class="iconfont ans-icon-clock" :class="'icon-' + el.state" v-if="el.state === 'RUNNING_EXECUTION'"
              data-toggle="tooltip" data-container="body" :title="$t('waiting')"></em>
          <em class="iconfont ans-icon-fail-solid" :class="'icon-' + el.state" v-if="el.state === 'FAILURE'"
      ...
      <span class="operation">
        <a href="javascript:" class="delete" @click="!isDetails && _remove($index)">
          <em class="iconfont ans-icon-trash" :class="_isDetails" data-toggle="tooltip" data-container="body"
              :title="$t('delete')"></em>
        </a>
