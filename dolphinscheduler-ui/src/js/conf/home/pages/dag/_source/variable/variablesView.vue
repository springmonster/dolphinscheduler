...
<template>
  <div class="variable-model">
    <template v-if="list">
      <div class="list">
        <div class="name"><em class="fa ans-icon-code"></em><strong
          style="padding-top: 3px;display: inline-block">{{ $t('Global parameters') }}</strong></div>
        <div class="var-cont">
      ...
      <div class="list" style="height: 30px;">
        <div class="name"><em class="fa ans-icon-code"></em><strong
          style="padding-top: 3px;display: inline-block">{{ $t('Local parameters') }}</strong></div>
        <div class="var-cont">
          &nbsp;
        </div>
      </div>
      <div class="list list-t" v-for="(item,key,$index) in list.localParams" :key="$index">
        ...
        <div class="var-cont" v-if="item.localParamsList.length">
          <template v-for="(el,index) in item.localParamsList">
            <x-button size="xsmall" type="ghost" :key="index" @click="_copy('copy-part-' + index)"
                      :data-clipboard-text="_rtClipboard(el,item.taskType)" :class="'copy-part-' + index">
